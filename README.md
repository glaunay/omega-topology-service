...
If the automatic rebuild is not disabled (`-d` option), the service will try, every `max_days_before_renew` days, to rebuild trees from CouchDB, in case the database has been refreshed.
